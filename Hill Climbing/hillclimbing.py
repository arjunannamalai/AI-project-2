...
repository (e.g., eil76.tsp, ch130.tsp, d198.tsp, etc.) and runs multiple hill climbing experiments.
...
import random
import time
import shutil  # New: to remove snapshot folders between runs
import matplotlib.pyplot as plt
import imageio.v2 as imageio

# Parsing the TSP file to get city coordinates
def parse_tsp_file(filepath):
    # If the provided path is not absolute, join it with the script's directory.
...
def hill_climbing(cities, max_iterations=10000, time_limit=600, snapshot_callback=None):
    ...
    while iteration < max_iterations and (time.time() - start_time) < time_limit:
        ...
        if neighbor_cost < current_cost:
            ...
            if current_cost < best_cost:
                ...
                # Capture only when an improvement is found.
                if snapshot_callback:
...
def run_experiments(tsp_file="ant_colony_opt_TSP/problems/eil76.tsp", num_runs=5,
                    ...
                    gifs_dir="results/gifs", export_gif_per_run=True):
    ...
    for run in range(1, num_runs + 1):
        ...
        if export_gif_per_run:
            snapshot_folder = os.path.join(plots_dir, f"hc_run_{run}_snapshots")
            # Clear snapshot folder if exists from a previous run
            if os.path.exists(snapshot_folder):
                shutil.rmtree(snapshot_folder)
            os.makedirs(snapshot_folder)

        # Define snapshot callback if needed
        ...
        # Run hill climbing with snapshot callback if enabled
        current_callback = snapshot_callback if snapshot_folder else None
        route, cost, iter_best, total_iter, runtime = hill_climbing(cities, max_iterations, time_limit, current_callback)
        ...
        # If GIF export is enabled, create a GIF for all snapshots of this run
        if export_gif_per_run:
            gif_path = os.path.join(gifs_dir, f"hc_run_{run}_animation.gif")
...
if __name__ == "__main__":
    random.seed(42)
    run_experiments(tsp_file="ant_colony_opt_TSP/problems/eil76.tsp", num_runs=5, max_iterations=10000, time_limit=600, plots_dir="results/plots", gifs_dir="results/gifs", export_gif_per_run=True)
